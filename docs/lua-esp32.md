...
This page documents **ESP32 peripheral simulation only**. Mobile Central and Observer Lua are documented separately in [Mobile Lua API]({{< relref "lua-mobile" >}}). 
Peripheral Lua in device packs may be edited on mobile, but it is currently **executed on the ESP32 simulator / firmware**, not on the phone. Functions such as `ble_notify`, `gfx_*`, and `adv_*` described here are therefore firmware-only.
